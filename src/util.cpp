...
/*
    ...
    * SLarew - prevent writing past array in Convert_multibyteUTF16_to_wchar
   bugfix
...
/*----------------------
findFileSize
  utf8_filepath - a pointer to a string (possibly utf8) of the full path to the
file

    take an ascii/utf8 filepath (which if under a unicode enabled Win32 OS was
already converted from utf16le to utf8 at program start) and test if AP is
running on a unicode enabled Win32 OS. If it is and converted to utf8 (rather
than just stripped), convert the utf8 filepath to a utf16 (native-endian)
filepath & pass that to a wide stat. Or stat it with a utf8 filepath on Unixen &
win32 (stripped utf8).
----------------------*/
uint64_t findFileSize(const char *utf8_filepath) {
  ...
  return 0; // won't ever get here.... unless this is win32, set to utf8 and the
            // folder/file had unicode.... TODO (? use isUTF8() for high ascii?)
}

/*----------------------
APar_OpenFile
  utf8_filepath - a pointer to a string (possibly utf8) of the full path to the
file file_flags - 3 bytes max for the flags to open the file with (read, write,
binary mode....)

    take an ascii/utf8 filepath (which if under a unicode enabled Win32 OS was
already converted from utf16le to utf8 at program start) and test if AP is
running on a unicode enabled Win32 OS. If it is, convert the utf8 filepath to a
utf16 (native-endian) filepath & pass that to a wide fopen with the 8-bit file
flags changed to 16-bit file flags. Or open a utf8 file with vanilla fopen on
Unixen.
----------------------*/
...
openSomeFile
  utf8_filepath - a pointer to a string (possibly utf8) of the full path to the
file open - flag to either open or close (function does both)

    take an ascii/utf8 filepath and either open or close it; used for the main
ISO Base Media File; store the resulting FILE* in a global source_file
----------------------*/
...
void TestFileExistence(const char *filePath, bool errorOut) {
  ...
  if ((a_file == NULL) && errorOut) {
    fprintf(stderr,
            "AtomicParsley error: can't open %s for reading: %s\n",
            filePath,
            strerror(errno));
    exit(1);
  } else {
    if (a_file == NULL) {
      fprintf(stderr,
              "AtomicParsley warning: can't open %s for reading but continuing "
              "anyway: %s\n",
              filePath,
              strerror(errno));
    } else {
...
const char *APar_strferror(FILE *f) {
  if (feof(f) && ferror(f))
    return "error and end of file";
  else if (feof(f))
    return "end of file";
  else if (ferror(f))
    return "error";
  else
    return "neither error nor end of file";
}
...
uint8_t APar_read8(FILE *ISObasemediafile, uint64_t pos) {
  ...
  if (size != 1) {
    printf("%s read failed, expect 1, got %u: %s\n",
           __FUNCTION__,
           (unsigned int)size,
           APar_strferror(ISObasemediafile));
    exit(1);
  }
  return a_byte;
}
...
APar_ReadFile(char *destination_buffer, FILE *a_file, uint32_t bytes_to_read) {
  ...
  if (destination_buffer != NULL) {
    fseeko(a_file, 0, SEEK_SET); // not that 2gb support is required - malloc
                                 // would probably have a few issues
    bytes_read = fread(destination_buffer, 1, bytes_to_read, a_file);
    file_size += bytes_read; // accommodate huge files embedded within small
                             // files for APar_Validate
  }
  return bytes_read;
}
  ...
  while (current_pos <= parsedAtoms[an_atom].AtomicLength) {
    ...
    if (eval_number > 65535) {
      ...
      if (APar_read32(uint32_buffer,
                      ISObasemediafile,
                      parsedAtoms[an_atom].AtomicStart + current_pos) ==
          eval_number) {
        break;
      }
    } else {
...
uint16_t PackLanguage(
    const char *language_code,
    uint8_t lang_offset) { //?? is there a problem here? und does't work
                           // http://www.w3.org/WAI/ER/IG/ert/iso639.htm
  // I think Apple's 3gp asses decoder is a little off. First, it doesn't
  ...
  // language_code[1+lang_offset], language_code[2+lang_offset]);

  if (language_code[0 + lang_offset] < 97 ||
      language_code[0 + lang_offset] > 122 ||
      language_code[1 + lang_offset] < 97 ||
      language_code[1 + lang_offset] > 122 ||
      language_code[2 + lang_offset] < 97 ||
      language_code[2 + lang_offset] > 122) {
...
#ifndef HAVE_STRSEP
// use glibc's strsep only on windows when cygwin & libc are undefined;
// otherwise the internal strsep will be used This marks the point where a
// ./configure & makefile combo would make this easier
...
char *APar_gmtime64(uint64_t total_secs, char *utc_time) {
  // this will probably be off between Jan 1 & Feb 28 on a leap year by a
  // day.... I'll somehow cope & deal.
  struct tm timeinfo = {0, 0, 0, 0, 0};

  int offset_year =
      (int)(total_secs / 31536000); // 60 * 60 * 24 * 365 (ordinary year in
                                    // seconds; doesn't account for leap year)
  int literal_year = 1904 + offset_year;
  int literal_days_into_year = ((total_secs % 31536000) / 86400) -
                               (offset_year / 4); // accounts for the leap year
...
char *APar_extract_UTC(uint64_t total_secs) {
  ...
  //    + 1,468,800 (60 * 60 * 24 * 17 leap days in 01/01/1904 to 01/01/1970
  //    duration)
  //= 2,082,844,800
  ...
  if (total_secs > MAXTIME_32) {
    ...
    if (total_secs < 2082844800) {
      ...
      strftime(
          *&utc_time, 50, "%a %b %d %H:%M:%S %Y", gmtime(&reduced_seconds));
      return *&utc_time;
    }
  }
  return *&utc_time;
...
APar_StandardTime
    ...
    Print the ISO 8601 Coordinated Universal Time (UTC) timestamp (in
...
void APar_StandardTime(char *&formed_time) {
  ...
  strftime(formed_time,
           100,
           "%Y-%m-%dT%H:%M:%SZ",
           timeinfo); // that hanging Z is there; denotes the UTC
...
wchar_t *
Convert_multibyteUTF16_to_wchar(char *input_unicode,
                                size_t glyph_length,
                                bool skip_BOM) { // TODO: is this like wcstombs?
  int BOM_mark_bytes = 0;
  if (skip_BOM) {
    BOM_mark_bytes = 2;
  }

  wchar_t *utf16_data = (wchar_t *)malloc(
      sizeof(wchar_t) *
      (glyph_length + 1)); // just to be sure there will be a trailing NULL
...
unsigned char *Convert_multibyteUTF16_to_UTF8(char *input_utf16,
                                              size_t glyph_length,
                                              size_t byte_count) {
  unsigned char *utf8_data =
      (unsigned char *)malloc(sizeof(unsigned char) * glyph_length);
  memset(utf8_data, 0, glyph_length);

  UTF16BEToUTF8(
      utf8_data, glyph_length, (unsigned char *)input_utf16 + 2, byte_count);
  return utf8_data;
}

wchar_t *Convert_multibyteUTF8_to_wchar(
    const char *input_utf8) { // TODO: is this like mbstowcs?
  wchar_t *return_val = NULL;
  size_t string_length = strlen(input_utf8) + 1; // account for terminating NULL
  size_t char_glyphs = mbstowcs(
      NULL,
      input_utf8,
      string_length); // passing NULL pre-calculates the size of wchar_t needed

  unsigned char *utf16_conversion =
      (unsigned char *)malloc(sizeof(unsigned char) * string_length * 2);
  memset(utf16_conversion, 0, string_length * 2);

  int utf_16_glyphs = UTF8ToUTF16BE(utf16_conversion,
                                    char_glyphs * 2,
                                    (unsigned char *)input_utf8,
                                    string_length) /
                      2; // returned value is in bytes
  return_val = Convert_multibyteUTF16_to_wchar(
      (char *)utf16_conversion, (size_t)utf_16_glyphs, false);
...
// 0x01 = UTF-16 with BOM. All frames have same encoding & terminate with
// 0x0000. 0x02 = UTF-16BE without BOM & terminate with 0x0000. 0x03 = UTF-8 &
// terminated with 0x00. buffer can hold either ut8 or utf16 carried on 8-bit
// char which requires a cast
/*----------------------
findstringNULLterm
  in_string - pointer to location of a string (can be either 8859-1, utf8 or
utf16be/utf16be needing a cast to wchar) encodingFlag - used to denote the
encoding of instring (derived from id3v2 2.4 encoding flags) max_len - the
length of given string - there may be no NULL terminaiton, in which case it will
only count to max_len

    Either find the NULL if it exists and return how many bytes into in_string
that NULL exists, or it won't find a NULL and return max_len
----------------------*/
uint32_t
findstringNULLterm(char *in_string, uint8_t encodingFlag, uint32_t max_len) {
  ...
  if (encodingFlag == 0x00 || encodingFlag == 0x03) {
    ...
    while (byte_count <= max_len) {
      wbufptr =
          (*(in_string + byte_count) << 8) | *(in_string + byte_count + 1);
      if (wbufptr == 0x0000) {
        break;
      }
      byte_count += 2;
...
uint32_t float_to_16x16bit_fixed_point(double floating_val) {
  ...
  // to get a fixed 16-bit decimal, work on the decimal part along; multiply by
  // (2^8 * 2) which moves the decimal over 16 bits to create our int16_t now
  // while the degrees can be negative (requiring a int16_6), the decimal
  // portion is always positive (and thus requiring a uint16_t)
  uint16_t long_decimal =
      (int16_t)((floating_val - long_integer) * (double)(65536));
  fixedpoint_16bit =
      long_integer * 65536 +
      long_decimal; // same as bitshifting, less headache doing it
...
bool APar_assert(bool expression,
                 int error_msg,
                 const char *supplemental_info) {
  bool force_break = true;
  if (!expression) {
    force_break = false;
    switch (error_msg) {
    case 1: { // trying to set an iTunes-style metadata tag on an
              // 3GP/MobileMPEG-4
      fprintf(stdout,
    ...
    case 2: { // trying to set a 3gp asset on an mpeg-4 file with the improper
              // brand
      fprintf(stdout,
              "AP warning:\n\tSetting the %s asset is only available on 3GPP "
              "files branded 3gp6 or later.\nSkipping\n",
              supplemental_info);
      break;
    }

    case 3: { // trying to set 'meta' on a file without a iso2 or mp42
              // compatible brand.
      fprintf(stdout,
              "AtomicParsley warning: ID3 tags requires a v2 "
              "compatible file, which was not found.\nSkipping.\n");
      break;
    }
    case 4: { // trying to set a 3gp album asset on an early 3gp file that only
              // came into being with 3gp6
      fprintf(stdout, "Major brand of given file: %s\n", supplemental_info);
      break;
    }
    case 5: { // trying to set metadata on track 33 when there are only 3 tracks
      fprintf(stdout,
              "AP warning: skipping non-existing track number setting user "
              "data atom: %s.\n",
              supplemental_info);
      break;
    }
    case 6: { // trying to set id3 metadata on track 33 when there are only 3
              // tracks
      fprintf(stdout,
              "AP error: skipping non-existing track number setting frame %s "
              "for ID32 atom.\n",
              supplemental_info);
      break;
    }
    case 7: { // trying to set id3 metadata on track 33 when there are only 3
              // tracks
      fprintf(stdout,
              "AP warning: the 'meta' atom is being hangled by a %s handler.\n "
              " Remove the 'meta' atom and its contents and try again.\n",
              supplemental_info);
      break;
    }
    case 8: { // trying to create an ID32 atom when there is a primary item atom
              // present signaling referenced data (local or external)
      fprintf(stdout,
              "AP warning: unsupported external or referenced items were "
              "detected. Skipping this frame: %s\n",
              supplemental_info);
      break;
    }
    case 9: { // trying to eliminate an id3 frame that doesn't exist
      fprintf(stdout,
              "AP warning: id3 frame %s cannot be deleted because it does not "
              "exist.\n",
              supplemental_info);
      break;
    }
    case 10: { // trying to eliminate an id3 frame that doesn't exist
      fprintf(stdout,
              "AP warning: skipping setting unknown %s frame\n",
              supplemental_info);
      break;
    }
    case 11: { // insuffient memory to malloc an id3 field (probably picture or
               // encapuslated object)
      fprintf(stdout,
              "AP error: memory was not alloctated for frame %s. Exiting.\n",
              supplemental_info);
      break;
    }
    }
